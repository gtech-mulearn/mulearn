...
const ManageUsers = (props: Props) => {
    ...
    const handleUserDelete = async (userId: string) => {
        try {
            ...
            setSelectedUsers(
                selectedUsers.filter(user => user.value !== userId)
            );
        } catch (err) {
            console.log(err);
        }
    ...
    useEffect(() => {
        (async () => {
            if (!!currRole.id) setSelectedUsers(await getUser(currRole.id));
        })();
    }, [currRole]);
    return (
        <Formik
            ...
            onSubmit={values => {
                addUsers(values.users, values.role);
                props.onClose(null);
            }}
                    ...
                    <ul className={styles.userList}>
                        {selectedUsers.map(user => (
                            <li>
                                ...
                                <button
                                    type="button"
                                    onClick={() => handleUserDelete(user.value)}
                                >
                                    <AiOutlineDelete />
                                </button>
                            </li>
