...
export const getWadhwaniCourses = async (clientToken: string) => {
    try {
        ...
        const message: any = response?.data?.response;
        console.log(message);
        return { response: message, error: null };
    } catch (err: unknown) {
        const error = err as AxiosError;
...
//   method: 'get',
//   maxBodyLength: Infinity,
//   url: 'https://dev.mulearn.org/api/v1/integrations/wadhwani/course-details/',
//   headers: {
